...
2. Generate separate schema-v2 files with `scripts/generation/generate_scalable_hdf5.py`.
...
4. Train with `bash run_zrs_mag.sh -m altermagnetism_LLG.scripts.training.train --input FILES...`.
5. Generate 96/128-cell held-out samples with `bash run_zrs_mag.sh -m altermagnetism_LLG.scripts.inference.sample`.
