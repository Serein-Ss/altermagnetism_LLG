...
>
> 本文规定五篇文献的原条件、特殊方程及逐项复现门槛；零场路径数据与生成模型的阶段、规模和放行按 [研究与验收协议修订4](PUBLICATION_GRADE_RESEARCH_PROTOCOL_20260911.md#scope) 执行。各体系复现任务独立保留；所用模型/共用内核的相关失败阻断依赖步骤，未使用的独立体系不作为主线前置门。本次不降低文献容差；未明确解决的冲突仍须记录并暂停依赖步骤。
