...
def test_historical_path_resolution(tmp_path, monkeypatch):
    monkeypatch.setattr(project_paths, "ROOT", tmp_path)
    (tmp_path / 'logs/restructure/history').mkdir(parents=True)
    (tmp_path / "logs/restructure/history/DIRECTORY_MIGRATION.json").write_text(json.dumps({"directory_rules": {"data/old": "data/research/datasets/new"}}))
    assert project_paths.resolve_recorded_path("data/old/a.h5") == tmp_path / "data/research/datasets/new/a.h5"


def test_lfs_manifest_destinations_and_sizes():
    manifest = json.loads((ROOT / "logs/restructure/history/LFS_SPLIT_MANIFEST.json").read_text())
