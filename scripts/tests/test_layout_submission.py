...
def test_submission_uses_categorized_snapshot_and_logs(tmp_path, monkeypatch):
    ...
    for name in ["scripts", "GUIDE", "slurm", "data/research/datasets/standard_v3/altermagnet_gomonay2024"]:
        (root / name).mkdir(parents=True)
    (root / "scripts/example.py").write_text("# snapshot example\n")
    (document_path(root / "GUIDE/plan.md")).write_text("# plan\n")
    (root / "slurm/guide_thermal_audit.sbatch").write_text("#!/bin/bash\n")
    (root / "data/research/datasets/standard_v3/registry.yaml").write_text("{}")
    (root / "data/research/datasets/standard_v3/altermagnet_gomonay2024/protocol.candidate.yaml").write_text("{}")
    (tmp_path / "run_zrs_mag.sh").write_text("# fake runner; never executed\n")
    output = root / "assets/research/audit_data/test_batch"
