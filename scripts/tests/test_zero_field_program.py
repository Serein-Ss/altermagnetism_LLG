...
def test_source_s1_field(periodic):
    r=load_runtime(ROOT/'conf/literature_reproduction/physics/gomonay_2024.yaml').reduced
...
def test_real_llg_shard_loader_and_no_training(tmp_path,orientation,basis):
    c=yaml.safe_load((ROOT/'conf/research/experiments/zero_field_gomonay/software_validation.yaml').read_text())
    c.update(steps=5,save_every=2,orientation=orientation,material_config=str(ROOT/'conf/literature_reproduction/physics/gomonay_2024.yaml'))
