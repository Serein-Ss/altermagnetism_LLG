...
def analyze_task(task,analysis_id):
    ...
    out=ROOT/'assets/literature_reproduction'/paper/run
...
def main():
    ...
    for task in ([] if args.finalize_only else m['tasks']):
    ...
    out=ROOT/'assets/literature_reproduction'/args.analysis_id;out.mkdir(exist_ok=args.finalize_only)
    if args.finalize_only:
        results=json.loads((out/'metrics.json').read_text())
        if {r['run_id'] for r in results}!={r['run_id'] for r in m['tasks']}:raise ValueError('Missing task metrics')
    else:write(out/'metrics.json',results)
