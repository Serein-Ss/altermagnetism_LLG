...
def main():
    ...
    out=ROOT/'assets/literature_reproduction'/aid;rows=json.loads((out/'metrics.json').read_text());comparisons=compare(rows,aid)
    write(out/'comparisons.json',comparisons)
    for paper in PAPERS:
        group=[r for r in rows if r['paper']==paper];doc=read_document(ROOT/'conf/literature_reproduction/physics'/f'{paper}.yaml')
        report=ROOT/'assets/literature_reproduction'/paper/aid;report.mkdir(exist_ok=False)
        ...
        with (document_path(report/'report.md')).open('x') as f:f.write(text)
        write(report/'report.json',dict(paper=paper,status='inconclusive',limitations=notes,tasks=group))
        for r in group:
            rdir=ROOT/'assets/literature_reproduction'/paper/r['run_id']/aid
            with (document_path(rdir/'report.md')).open('x') as f:
    ...
    write(out/'report_artifacts.json',dict(code_sha256=sha256(Path(__file__)),files={str(p.relative_to(ROOT)):sha256(p)
        for folder in (ROOT/'assets/literature_reproduction',ROOT/'assets/literature_reproduction')
