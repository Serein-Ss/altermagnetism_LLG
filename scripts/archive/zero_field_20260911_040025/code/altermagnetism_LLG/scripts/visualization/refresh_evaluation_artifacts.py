...
from altermagnetism_LLG.scripts.analysis.evaluate import (  # noqa: E402
    _json_row,
    _plot_histograms,
    _plot_paths,
)
...
from altermagnetism_LLG.scripts.core.project_paths import generated_path, resolve_recorded_path
...
def refresh(directory: Path, system: str) -> None:
    ...
    trajectory_file = generated_path(directory / "generated_trajectories.h5")

    with h5py.File(resolve_recorded_path(report["input"]), "r", swmr=True) as source, h5py.File(
        trajectory_file, "r", swmr=True
    ) as generated:
