...
def main():
    ...
    code = output / "code" / "altermagnetism_LLG"
    code.mkdir(parents=True)
    for name in ("scripts","model","GUIDE"):
        shutil.copytree(ROOT/name,code/name,ignore=shutil.ignore_patterns("__pycache__","*.pyc",".pytest_cache"))
    for name in ("registry.yaml","altermagnet_gomonay2024/protocol.candidate.yaml"):
        target=code/"data"/"standard_v3"/name
        target.parent.mkdir(parents=True,exist_ok=True)
        shutil.copy2(ROOT/"data"/"standard_v3"/name,target)
    shutil.copy2(ROOT.parent/"run_zrs_mag.sh",output/"code"/"run_zrs_mag.sh")
    batch=output/"guide_thermal_audit.sbatch"
