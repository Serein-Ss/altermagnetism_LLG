...
def prepare():
    ...
    for index, dt in enumerate((.02, .01)):
        ...
        c['implementation_notes']['trajectory'] = 'duration=750000, save_dt=1 reduced; saving convergence still pending'
        ...
        result = subprocess.run(command, cwd=OLD, capture_output=True, text=True)
