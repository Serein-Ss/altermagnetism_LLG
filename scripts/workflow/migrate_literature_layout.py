...
def main():
    manifest=ROOT/'logs/restructure/history/LITERATURE_DIRECTORY_MIGRATION.json'
    ...
    def add(old,paper,kind,filename=None):
        ...
        moves[old]=dest
    add('data/literature_reproduction/legacy/literature_validation/Gomonay_2024_main.pdf','gomonay_2024','reference')
    add('data/literature_reproduction/legacy/literature_validation/bauer_2011_magnetic_chain.pdf','bauer_2011','reference')
    add('data/literature_reproduction/legacy/literature_validation/spinwave_trajectory.npz','gomonay_2024','raw')
    add('data/literature_reproduction/legacy/literature_validation/spinwave_validation.json','gomonay_2024','derived')
    for p in sorted((ROOT/'data/literature_reproduction/legacy/path_literature_validation').iterdir()):
        kind='raw' if p.suffix=='.h5' or p.name.endswith('.manifest.json') else 'derived'
        add(str(p.relative_to(ROOT)),'bauer_2011',kind)
    for p in sorted((ROOT/'data/literature_reproduction/legacy/noncollinear_validation/crnb3s6').iterdir()):
        add(str(p.relative_to(ROOT)),'laliena_crnb3s6_2020','raw' if p.suffix=='.npz' else 'derived')
    for p in sorted((ROOT/'assets/literature_reproduction/legacy/literature_validation').rglob('*')):
    ...
    composite='assets/literature_reproduction/legacy/next_stage_validation/literature_path_validation.png'
    ...
    for paper in PAPERS:
        ...
        status={'paper_id':paper,'legacy_run_id':LEGACY,'legacy_status':'not_strictly_certified',
                ...
                'config':f'conf/literature_reproduction/physics/{paper}.yaml'}
        output=ROOT/'assets/literature_reproduction'/paper/LEGACY
