...
def main():
    config=ROOT/'conf/literature_reproduction/physics'/f'{PAPER}.yaml';base=read_document(config)
    for file in sorted((ROOT/'conf/literature_reproduction/physics').glob('*.yaml')): read_document(file)
    ...
    noise=ROOT/'assets/literature_reproduction'/PAPER/'preflight_20260909/noise_gate.json'
    ...
    source_files+=['conf/reduced_llg.yaml']+[str(p.relative_to(ROOT)) for p in sorted((ROOT/'conf/literature_reproduction/physics').glob('*.yaml'))]
    ...
    output=ROOT/'assets/literature_reproduction'/PAPER/campaign_id;output.mkdir(parents=True)
