...
def prepare():
    ...
    files += list((ROOT/'conf/literature_reproduction/physics').glob('*.yaml'))
    hashes={}
    for src in files:
...
def run_task(campaign,kind,index):
    ...
    if kind=='gpu_check':
        ...
        for paper in PAPERS:
            subprocess.run([sys.executable,'-m',f'scripts.literature.{paper}.run','--config',str(snapshot/'conf/literature_reproduction/physics'/f'{paper}.yaml'),
    ...
    out=root/'assets/literature_reproduction'/task['paper']/task['run_id']
