...
def run_phase(root,base,phase,config_dir,snapshot,workers,selected_factor=None):
    ...
    output=root/'assets/literature_reproduction'/PAPER/run_id;output.mkdir(parents=True)
    logs=root/'logs/literature_reproduction'/PAPER/run_id;logs.mkdir(parents=True)
    write_json(output/'log_location.json', dict(logs=str(logs)))
    ...
    (document_path(output/'report.md')).write_text(markdown_report(report,run_id))
...
def main():
    ...
    config_dir=root/'conf/archive/frozen'/PAPER/campaign.name;config_dir.mkdir(parents=True)
    ...
    destination=root/'assets/literature_reproduction'/PAPER/campaign.name
