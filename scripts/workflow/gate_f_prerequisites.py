...
SOURCE_FILES = [
    ...
    'conf/literature_reproduction/physics/gomonay_2024.yaml', 'conf/research/experiments/gate_f/prerequisites.yaml',
...
def prepare():
    design = yaml.safe_load((ROOT/'conf/research/experiments/gate_f/prerequisites.yaml').read_text())
    run_id = datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')
    output = ROOT/'assets/research/runs/gate_f/prerequisites'/run_id
    configs = ROOT/'conf/archive/frozen/gate_f_prerequisites'/run_id
    ...
    material = snapshot/'conf/literature_reproduction/physics/gomonay_2024.yaml'
    ...
    old_gate = ROOT/'conf/archive/frozen/zero_field_gomonay/20260911_040025/equilibrium_gate.json'
