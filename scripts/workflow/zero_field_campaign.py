...
def prepare():
    ...
    campaign=ROOT/'conf/archive/frozen/zero_field_gomonay'/stamp;campaign.mkdir(parents=True)
    ...
    files+=list((ROOT/'conf/literature_reproduction/physics').glob('*.yaml'))+list((ROOT/'conf/research/experiments/zero_field_gomonay').glob('*'))
    files+=list((ROOT/'slurm').glob('zero_field*.sbatch'))+list((ROOT/'GUIDE').glob('*.md'))
    files += list((ROOT/'data/research/datasets/standard_v3').rglob('*.yaml'))
    files += [ROOT/'logs/restructure/history/LFS_SPLIT_MANIFEST.json',ROOT/'assets/literature_reproduction/nishino_miyashita_2015/runs/20260909_8df33ac_bb48dc64/report.json']
    ...
    out=ROOT/'assets/research/runs/zero_field_program'/stamp;out.mkdir(parents=True)
    ...
    for index,task in enumerate(tasks):
        ...
        if task['kind'] in ('literature','nishino'):
            doc=read_document(snapshot/'conf/literature_reproduction/physics'/f'{task["paper"]}.yaml')
            ...
            doc=dict(material_config=str(snapshot/'conf/literature_reproduction/physics/gomonay_2024.yaml'),length=4,theta=task['theta'],
            ...
            doc=yaml.safe_load((snapshot/'conf/research/experiments/zero_field_gomonay/software_validation.yaml').read_text())
            doc.update(role='equilibrium_diagnostic',run_id=task['run_id'],
                material_config=str(snapshot/'conf/literature_reproduction/physics/gomonay_2024.yaml'),theta=task['theta'],shape=task['shape'],
        ...
        key=json.dumps(dict(kind=task['kind'],paper=task.get('paper'),protocol=task.get('protocol'),pool=task['pool'],
            ...
            nishino_case=task.get('task',0)//4 if task['kind']=='nishino' else None,
    ...
    bindings=dict(source_tree=hashlib.sha256(json.dumps(hashes,sort_keys=True).encode()).hexdigest(),
                  material=sha256(snapshot/'conf/literature_reproduction/physics/gomonay_2024.yaml'),
                  acceptance=sha256(snapshot/'conf/research/experiments/zero_field_gomonay/acceptance.yaml'))
    m=dict(root=str(ROOT),campaign=str(campaign),snapshot=str(snapshot),output=str(out),logs=str(log),
        ...
        acceptance=str(snapshot/'conf/research/experiments/zero_field_gomonay/acceptance.yaml'),
...
def sbatch(m,mode,*,indices=None,pool='cpu',dependency=None,minutes=30):
    ...
    command+=[str(Path(m['snapshot'])/'slurm/zero_field_campaign.sbatch'),m['root'],m['campaign'],mode,m['snapshot']]
