...
当前 flow matching 网络输入和输出都带有固定的时间帧维度。它不会仅凭温度、尺寸或阻尼自动推断任意物理时长；当前实现应表述为“在给定条件下生成固定窗口的路径”。不同条件使用不同总时长时，必须先切分或重采样到统一训练窗口，并保留原始时间轴。未经新增时间条件和滚动验证，不得声称支持任意长度。

### 16.2 训练数据如何组织

从每条原始 LLG 轨迹抽取长度为 $F_{train}$ 的连续窗口 $(\mathbf S_{t_0},\ldots,\mathbf S_{t_0+(F_{train}-1)\Delta t})$。窗口起点按初态 ID 分组，不能让同一物理轨迹的相邻窗口跨越 train/validation/test。训练阶段可以随机抽取窗口以增加样本数，但独立统计单位仍是完整轨迹和初态，不是窗口数量。

训练集的每个样本包含：

```text
initial_state        S(t0)
target_path          S(t0:t0+H)
physical_time_grid   [t0, ..., t0+H]
scalar_condition     [T, alpha, lag, J ratios, K, size, boundary]
vector_condition     [crystal axes, DMI vectors, optional field]
geometry             bond graph, masks, sublattice map
metadata              system/initial/noise IDs and hashes
```

训练目标是条件路径分布，不是单一标签。对同一个 `initial_id` 的多个噪声复制，模型学习其条件随机分支；对多个 `initial_id`，模型学习初态依赖；对多个条件块，模型学习温度、阻尼、尺寸和相互作用的变化。

### 16.3 模型输入、条件嵌入和输出

模型主输入是参考路径中的自旋场 $x_\tau$，形状为 `[batch, frames, sublattice, nx, ny, 3]`，以及生成运输时间 $\tau\in[0,1]$。条件分为三类：

1. 标量条件：$T$、$T/T_N$、$\alpha$、物理 lag、$J_2/J_1$、$\tilde J/J_1$、$K/J_1$、尺寸和边界编码；先用训练集均值和标准差标准化。
2. 向量条件：晶轴、DMI 向量、外场方向（零场时数值为零），只能通过协变基进入网络。
3. 几何条件：键表、开放边界 mask、子晶格和晶向映射，由图消息传递或 masked convolution 编码。

标量条件经 MLP/FiLM 调制残差块，向量条件与自旋构造点积、叉积和投影后的协变基。网络输出每个时间帧和每个格点的切向速度：

$$
v_\phi(x_\tau,\tau\mid c)\in T_{x_\tau}\mathcal M,
\qquad x_\tau\cdot v_\phi=0.
$$

ODE 求解器从参考分布积分 $\tau=0$ 到 $1$，输出与请求窗口相同帧数的自旋路径，以及能量、序参量等派生量。输出不是下一帧的确定值，而是由参考噪声和条件共同决定的一条随机路径；重复采样得到路径集合。

### 16.4 变长路径的实现路线

分三阶段实现，不能把尚未实现的能力写入论文：

* **阶段一，固定窗口**：所有条件统一为 $F_{train}$ 帧和固定物理 lag，只验证条件路径分布。
* **阶段二，多 lag 条件模型**：把目标物理窗口 $H$ 或 lag $\Delta t$ 作为显式输入，训练多个预注册 lag；测试未见 lag，并比较短段拼接与直接长段生成。
* **阶段三，滚动生成**：模型输出一个固定长度 chunk，使用末帧作为下一 chunk 初态，累积得到长路径。必须检查 chunk 边界能量、序参量、ACF、漂移和转移核一致性；滚动不是自动保持正确稳态。

温度或尺寸改变时，模型只能在训练条件覆盖或明确外推范围内请求对应长度。尺寸改变还要求网络支持可变空间维度和正确的 bond graph；仅改变张量大小不能证明物理泛化。

### 16.5 训练和测试分别使用什么数据

训练使用 `zero_field_temperature/train` 中完整轨迹切出的窗口；验证使用不同初态和噪声 seed 的验证块；测试使用整块留出的温度、尺寸、晶向或材料。文献驱动数据和 `legacy_v2_driven` 不进入零场模型训练。测试时从测试初态输入模型，生成与参考相同的短窗口，再用独立参考轨迹比较分布、动力学和稳态指标。长时 rollout 只在短时转移通过后进行。
