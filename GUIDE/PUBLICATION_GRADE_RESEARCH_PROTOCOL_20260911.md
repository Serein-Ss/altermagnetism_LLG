...
## 15. 参数来源、轨迹时长和相互作用分层说明

### 15.1 哪些是文献参数，哪些是本研究设计

Gomonay 论文直接约束的是哈密顿量形式、$J_1,J_2,\tilde J$、自旋波/畴壁各向异性、晶格几何、晶向和特定理论曲线。晶向 100/110 不是任意数据增强，而是论文中不同晶格取向和元胞映射的复现对象。

下列参数不是论文唯一规定值，而是为了统计和泛化而预注册的研究设计：$L=16,32,64,128$、$T/T_N$ 的八个温度层、$\alpha=0.01,0.05,0.10$、三类初态、每个初态至少八个实例、每个实例 32 个噪声复制。这些选择的依据是覆盖低温到临界区、区分尺寸效应、估计阻尼敏感性并避免把一条轨迹当重复；若资源或收敛分析表明不足，应依据事先定义的功效/置信区间重新计算，而不是把它们伪装成文献值。

### 15.2 轨迹物理时长

没有一个适用于所有尺寸和温度的固定时长。短时轨迹覆盖进动周期和局部转移，长时轨迹覆盖自相关衰减、稳态和事件率。建议每个条件先做 pilot，估计主进动周期 $\tau_{prec}$、能量/序参量自相关时间 $\tau_{corr}$ 和事件时间 $\tau_{event}$，再设置

$$
T_{short}\ge 20\tau_{prec},\qquad T_{long}\ge 50\tau_{corr},
$$

若研究跃迁，则要求观测窗足以得到预注册事件数或明确的删失上限。$T_N$ 附近要增加长时，因为临界减速会使 $\tau_{corr}$ 变大。1001 帧只是保存网格的工程默认值，用于在固定文件大小下解析短时波形；它不是物理定律，也不保证达到平衡。长时数据可以使用 1001–5001 帧，但必须同时写明保存间隔和总物理时长。

### 15.3 晶向如何进入实际计算

晶向已经进入 Gomonay 的物理计算，不是只在保存时写入标签。`model.py` 根据 orientation 选择元胞基矢、基点偏移、键模板、邻接表和坐标；有效场由这些键直接求和。因此 100 与 110 会产生不同的局域场、子晶格数和允许的波矢。通用 `ReducedLLG` 积分器本身不读取晶向，因为它只接收已经构造好的 Hamiltonian；若网络或数据加载器只把晶向作为字符串而没有改变邻接/键张量，则属于实现错误，必须用同一自旋状态的场差异测试发现。

### 15.4 为什么 DMI 和阻挫先分开

分开不是物理上认为它们互不相容，而是因果归因实验。若一开始同时加入 DMI、竞争交换、复杂边界和高温，性能变化或失败无法判断来自手性项、简并能谷、长相关时间还是代码邻接错误。无 DMI/无阻挫、仅 DMI、仅阻挫、联合体系构成逐项增加复杂度的消融阶梯；只有联合模型通过后，才能讨论二者耦合产生的新现象。最终仍需运行联合 DMI+阻挫数据，不能把两个独立结果当成联合物理结论。

### 15.5 原始 Gomonay 哈密顿量是否包含 DMI 或阻挫

当前复现的 Gomonay 原始模型显式使用 $J_1$、$J_2$、$\tilde J$ 和各向异性项，并未在代码对应的论文哈密顿量中加入独立的 DMI 向量项 $\mathbf D_{ij}\cdot(\mathbf S_i\times\mathbf S_j)$。其中 $J_2$ 等竞争交换可能改变色散和稳定性，但不能仅凭存在多个交换常数就称体系处于“阻挫相”；是否阻挫必须由晶格闭环、符号/比值、基态简并和结构因子验证。DMI 与阻挫扩展属于本项目新增挑战集，不能声称是 Gomonay 原文复现的一部分。
...
## 14. 温度数据分层与 V2 隔离放行规则

当前数据中的温度并不构成一个可直接用于温度泛化的干净矩阵。V2 只有物理温度 5 K，且每个尺寸配有 0.70、0.78、0.80 T 阻尼型 SOT 脉冲；因此它的路径主要是“5 K + 强驱动”条件。Nishino、Bauer 和 Hirst 的温度属于各自论文的约化温度或材料温度，模型、单位和哈密顿量不同，不能拼接成一个统一温度数据集。

数据必须分为三个不可混用的域：

| 域 | 内容 | 允许用途 |
|---|---|---|
| `literature_driven` | 文献要求的外场、SOT、电流或脉冲 | 文献复现、驱动动力学基线 |
| `legacy_v2_driven` | 5 K、强 SOT、短时二维 Gomonay 路径 | 代码调试和历史模型回归 |
| `zero_field_temperature` | 外场、SOT、STT、电流和脉冲全部为零 | 正式训练、温度效应和物理正确性判断 |

`legacy_v2_driven` 不得进入零场模型的训练、验证或测试，也不得用于估计温度噪声强度、稳态分布、$T_N$ 或温度泛化。若用于代码 sanity check，报告必须明确这是带驱动数据。

正式零场温度矩阵以当前模型自身的 $T_N$ 为基准，而不是直接把 V2 的 5 K 当作低温参考。先运行无外场热平衡扫描，得到尺寸相关的 $T_N(L)$；再使用 $T/T_N(L)$ 的分层温度，并同时保存 Kelvin、约化温度 $k_BT/J_1$ 和参考能量尺度。温度至少覆盖 $0.1,0.3,0.5,0.7,0.9,1.0,1.1,1.3\,T_N$，临界区增加初态数、噪声复制和物理时长。

模型输入必须显式区分：

```text
temperature_K
reduced_temperature = k_B*T/E0
T_over_TN
drive_field_like
drive_damping_like
drive_current
...
零场正式模型中后三项驱动值必须为零，并由数据证书强制检查。训练时温度特征只用训练集统计量标准化；测试集的 $T_N$ 估计、均值和方差不能反向进入训练。温度改变时，模型应改变热噪声和路径分支统计，而不是把温度当作标记标签记忆。

放行标准是：同一初态的独立噪声复制在低温、中温和临界区分别通过噪声方差、能量分布、序参量、相关时间和时间步收敛；模型在留出温度块上同时通过逐点、转移分布和稳态统计测试。任何带强驱动的数据只能作为单独域的迁移实验，不能作为零场正确性的证据。
