...
> 版本：2026-09-09；2026-09-13同步修订职责、证据保留、依赖停止规则及Bauer时间/噪声/寿命说明
>
> 适用仓库：`altermagnetism_LLG`
>
> 本文规定五篇文献的原条件、特殊方程及逐项复现门槛；零场路径数据与生成模型的阶段、规模和放行按 [研究与验收协议修订5](PUBLICATION_GRADE_RESEARCH_PROTOCOL_20260911.md#scope) 执行。各体系复现任务独立保留；所用模型/共用内核的相关失败阻断依赖步骤，未使用的独立体系不作为主线前置门。本次不降低文献容差；未明确解决的冲突仍须记录并暂停依赖步骤。
...
7. 一个体系失败时不得用另一体系成功替代；该体系及依赖同一缺陷的数据生成不得放行。独立未用体系的失败不阻断已满足自身及共用前置门的主线；五体系整体结论仍须五套证书。
...
原文Eq.(2)以`hbar*∂S/∂t`定义时间，故该约定采用`t0=hbar/E0`。上节通用`mu0/(gamma*E0)`仅在明确`mu0/gamma=hbar`映射时表示相同单位；此处mu0是磁矩，不是真空磁导率。若采用其他材料时间标定须单独登记，不静默换算原文寿命。
...
以下为旧结果及对应图片的历史路径。保留原始文件、失败和接受偏差记录；在清单中标记旧模型/旧方法及不适用范围，不作为新认证证据：
...
先登记原路径、run_id和hash；需要归档移动时保留映射与可访问性。新脚本/配置测试及数值认证通过后在新目录生成新结果，不覆盖旧数据或静默混用。撤销本节旧版的删除指令；本次文档更新不执行数据移动、删除或重跑。
...
### 7.0 修订5的适用范围：训练认证与完整复现分开

本节7.1–7.3保留完整Bauer原条件复现的目标与已有门槛，不再作为小体系生成模型训练的全部前置。主线按[主协议修订5](PUBLICATION_GRADE_RESEARCH_PROTOCOL_20260911.md#scope)执行：同参数小体系短轨迹通过方程/基础物理/数值/覆盖审计后可用于开发；冻结模型在未见大体系长时间条件下产生预测，再与文献统计对标。不能把小体系训练数据称为已完成文献大体系复现，也不能把未完成整篇扫描视为已发现求解器错误。

7.2禁止将改参数快速先导冒充原条件证据，不禁止清楚标注的小体系训练扩展。7.3的500完成事件是既有完整复现任务门槛，不是小体系训练集必须包含的事件数，更不是启动训练前必须完成的大体系模拟预算。生成模型外部检验按主协议5.3预注册样本精度、目标与停止规则；不以此重标旧HOLD为PASS。

### 7.1 严格目标

来源：J. Phys.: Condens. Matter 23, 394204 (2011)。使用论文开放链随机 Landau–Lifshitz 方程：`L=100`、`K/J=0.1`、`theta=k_BT/J=0.11`、`lambda=0.1`、本项目复现窗口 `750000 hbar/J`（数值/观察选择，不宣称原文规定统一总时长），无外场、无 SOT、无重要性筛选。
...
论文没有报告积分时间步和随机种子，因此当前 `dt=0.02` 不是文献参数。必须以 `0.02, 0.01, 0.005` 做收敛扫描；Bauer弱随机RK按其离散随机变量与弱统计验证，可独立抽样或使用经推导的合法耦合，不强制把它解释为共同Wiener强路径。只有选用支持Wiener路径的随机方法做强收敛时才用嵌套增量。只有观测量收敛后才能冻结生产步长，不得把较大时间步称为“论文原始设置”。

### 7.3 样本量与验收

- 静态检查：离散畴壁能与连续近似仅作为哈密顿量单元测试，不是动力学复现；
- 轨迹检查：所有轨迹等权保存，反转前后路径完整，不只保存成功事件；
- 原文寿命对照：按附录A计算长轨迹中相邻反转间隔的平均，每条件至少500个完成反转事件的既定门槛保留；固定初态首达另报Kaplan–Meier/RMST及删失，不能混为原文寿命。先冻结观察长度/重复数及最多补样规则，预算内未达到事件门槛记HOLD，不无限运行至显著；数值失败不当作物理右删失。
...
以下R0–R6组织五篇文献的独立复现；当前路径生成主线的先后关系采用新协议P0–P8，不强制先完成所有R任务再进入Bauer研究。
...
R6  汇总跨体系认证矩阵；五体系整体结论需全部通过，单体系主线按相关依赖放行
```

停止规则：R0或Nishino失败若涉及共用噪声/积分器等模块，停止依赖同一缺陷的有限温生产；影响范围未明确时暂停可能受影响的步骤并审计。已确认独立的未用模型失败不阻断本主线。某模型邻接/场错误阻断该模型及依赖它的步骤；长时统计不足记`inconclusive`，不能按pass处理。
